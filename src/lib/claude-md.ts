...
export function buildClaudeMd(brand: Brand | null, chosen: Variant | null): string {
  const lines: string[] = [
    ...
    "- Conjured imagery (max 3 per design, only after the page is built). ALWAYS wrap in `timeout`",
    "  so a slow provider can't hang the build, and keep it web-sized (NOT --image-size 2K):",
    "  `timeout 150 bash $HOME/.phantom-plugins/claude-image-generation/scripts/gemini.sh --mode generate --prompt \"...\" --aspect-ratio 16:9 --output public/assets/<fresh-slug>.png`",
    "  (or xai.sh; edit with `--mode edit --input-image public/assets/<file>`). If it times out or errors,",
    "  skip that image and use a tasteful non-image treatment — retry at most once. Fresh slugs only —",
