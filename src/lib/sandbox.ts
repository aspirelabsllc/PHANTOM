...
export const AGENT_RUNNER = [
  ...
  "  }",
  "}",
  "const transient = (e) => /connection closed|overloaded|rate limit|too many requests|429|500|502|503|504|529|ECONNRESET|ETIMEDOUT|socket hang up|terminated|fetch failed/i.test(String((e && e.message) || e));",
  "let attempt = 0;",
  "while (true) {",
  "  try {",
  ...
  "    if (transient(e) && attempt < 3) { attempt++; emit({ t: 'text', text: '(Connection dropped — resuming the build…)' }); await new Promise((r) => setTimeout(r, 1500 * attempt)); continue; }",
  "    emit({ t: 'error', message: String((e && e.message) || e) }); break;",
  "  }",
  "}",
  "emit({ t: 'end' });",
].join("\n");
